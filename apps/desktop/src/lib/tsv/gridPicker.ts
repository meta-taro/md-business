...
/**
 * 候補リストを開く前に、そのセルの下へ空けておく高さ（px）。
 *
 * リストが下へ開くか上へ開くかは WebView が決めるので、こちら側からは指示できない。
 * 表の下端の行はそのまま開くと画面の外へ出て選べないため、開く前に下へ余白を作る。
 * 余白が表示領域に収まらないときは行が上端へ寄る（＝下に取れるだけ取る）。
 */
export const PICKER_BOTTOM_GAP = 220;
