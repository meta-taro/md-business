...
/**
 * Sanitize the Markdown→HTML output before it is injected into the prose
 * preview pane (spec / test-spec). The desktop app renders the body inside a
 * Tauri webview `<iframe srcdoc>`; the app CSP already blocks remote scripts,
 * but the Markdown body can still smuggle XSS via inline `<svg>` event handlers
 * or `javascript:` URLs. We harden the surface with an explicit allowlist:
 *
 ...
 *
 * DOMPurify needs a DOM. The desktop preview runs in a Tauri webview, so
 * `window.document` is always present there. Vitest uses `jsdom` for the prose
 * provider tests (per-file `// @vitest-environment jsdom` docblock), which also
 * provides `window`. (chrome-extension tried `happy-dom` first but its HTML
 * parser silently dropped leading `<h1>` elements when parsing body fragments,
 * breaking any spec whose first node is `# 概要`.) We pass `globalThis.window`
 * explicitly to a lazily-built DOMPurify instance so a missing window throws a
 * clear error instead of a confusing "factory is not a function".
 *
 * NOTE: kept in sync with apps/chrome-extension/src/shared/sanitizeHtml.ts. The
 * security policy (URI allowlist / forbidden tags) MUST stay identical across
...
function getPurify(): PurifyInstance {
  ...
  if (!win) {
    throw new Error('sanitizeViewerHtml requires a window (Tauri webview or jsdom test env).');
  }
...
export interface SanitizeViewerHtmlOptions {
  /**
   * When true, accept inline `<svg>` elements. Defaults to true — the prose
   * viewer explicitly wants diagrams. Set to false for narrower contexts (e.g.
   * a future "trust = none" preview mode).
   */
  allowSvg?: boolean;
}
