...
**ファイル名は日本語が既定**。業務文書は日本語名で運用されるため、表示・絞り込みとも日本語を一級で扱う。

- **並び順**: `localeCompare`（読み順）。コードポイント順ではひらがな・カタカナ・漢字が読みと無関係に並ぶ。
- **名前は省略しない**: 長い日本語名は `…` で切らず、横スクロールで全名を見せる（`.tree` が縦横 auto）。似た名前が並ぶ業務文書では、末尾（「_控え」「_v2」）が判別の要になるため。
- **絞り込みの照合は NFKC + 小文字化**。見た目が同じで内部表現が違うものを一致させる: 濁点の合成揺れ（`タ`+濁点 / `ダ`）、IME を on にしたまま打った全角英数（`ＡＰＩ` / `API`）、半角カナ（`ｼﾌﾄ` / `シフト`）。畳むのは**照合キーだけ**で、表示名は元のまま。
- **IME 変換中の Escape は奪わない**。Escape は変換の取り消しに割り当たっているため、`isComposing` 中は入力欄をクリアしない。確定後の Escape だけがクリアになる。

**最近開いたフォルダ**。作業フォルダは数個を行き来するのが実態なので、DB クライアントの接続先や git クライアントのリポジトリ一覧と同じく、過去に開いたフォルダを一覧から選べるようにする。

- **置き場所は 2 つ**: フォルダ未選択の空状態では一覧をそのまま並べる（起動直後にダイアログを辿らせない）。フォルダを開いている間はヘッダーの `▾` から重ねて出す（常設するとツリーの場所を奪うため）。
- **1 行の内容**: フォルダ名 + 親パス。同名フォルダ（`docs` が複数）を名前だけでは見分けられないため。親パスは補助情報として小さく右寄せ、入り切らない分は省略し、全体は `title` で見せる。
- **削除は行ホバーで出る `✕`**。常時表示だと選ぶときに押し間違えるため、触れるまで出さない。
- **消えたフォルダは畳まず印を付ける**。移動・削除だけでなく共有ドライブの切断でも起きるので、勝手に忘れると繋ぎ直したときに戻せない。取り消し線 + 「見つかりません」バッジで示し、消すかどうかは利用者が決める。
- **開く直前にもう一度存在を確かめる**。一覧を出してから選ぶまでの間に消えることがある。走査エラーで気付かせるより、一覧上で消えていると分かるほうが直しやすい。

**十字キーで動かせる**。ツリーは行数が多くなりやすく、マウスに持ち替えずに辿れることが要る。エクスプローラー / IDE の作法（WAI-ARIA の tree パターン）に合わせる。

- **上下**は行送り、**左右**は階層の出入り。右は畳んだフォルダを開き、開いていれば最初の子へ入る。左は開いたフォルダを畳み、畳んでいれば親へ戻る。ファイルの右は何もしない（開く手段は Enter で足りる）。
- **端では止める**。押しっぱなしで先頭・末尾を巻き込むと、どこにいるか分からなくなる。先頭・末尾へ飛ぶのは Home / End に分ける。
- **Tab の入口は 1 つ**（roving tabindex）。行ごとに Tab が止まると、ツリーを抜けるだけで行数分の Tab を押すことになる。
- **Enter / Space は既定動作のまま**。行は `button` なので、開閉・選択はブラウザの活性化に任せる（キー処理を重ねると IME や支援技術の挙動とずれる）。
- **絞り込み中は移動だけ**。絞り込み結果は全展開の一時ツリーなので、そこで畳むと絞り込みの意味が消える。

### 5.8 検証グリッド（スプレッドシート編集面）

§5.3 の読み取り表（プレビュー）とは役割が異なる。検証シートの TSV 編集グリッド（`TsvGrid`）は **QA が Office / Workspace なしで検証を完了する本命の入力面**であり、見た目・操作感を **スプレッドシート（Excel / Google Sheets）に寄せる**（まるでスプレッドシートと同様の使用感にする要件）。
...
- **行の条件付き書式**: `#@ style <列名> <値>=<色>` で指定列の値に応じて**行全体**を薄く塗る（例: 結果 OK / NG / 保留）。色は tsv 側に持つのでシートごとに凡例と塗りを揃えられる。既定テンプレートは §2.3 のセマンティック地（`--success-bg` / `--danger-bg` / `--warning-bg`）と同じパステルを使う。**塗りは地であって情報ではない**（値そのものがセルに出ているので、色覚に依存させない）。**ダークテーマでは色を作り直す**（`--row-tint-bg`）。tsv の色はライトの地を前提にしたパステル（＝明るくて淡い）なので、暗い地にそのまま敷くと白飛びして文字が読めない。逆に薄い被膜まで落とすと、元が淡いぶん色味まで消えて全行が同じ灰色になり、行色が無いのと変わらなくなる。そこで**明度は暗い地に合わせて置き換え、彩度は持ち上げて色の違いだけを残す**（相対色 `oklch(from …)`。派手な色を書かれても浮かないよう彩度には上限を掛ける）。既定状態（未実施）は塗らない＝全行が色付きになると帯の意味が消えるため。
...
**Top bar のアクション群（§6.5 のタイトルバー右側）** は幅に応じて 3 段で縮退する。タイトルバーを兼ねる帯なので、ラベルの折り返しは帯の高さと文書名の位置を崩す＝**折り返しは禁止**。

- **> 1120px**: アイコン + ラベル全文。
- **1120px 以下**: ラベルを詰め、溢れた分は末尾を `…` で省略。
- **900px 以下**: ラベルを落として**アイコンのみ**。ブランド名も隠し（ドットは残す）、帯の幅を文書名に譲る。

アイコンとクリック領域は最後まで残す。ラベルが消えても `title` / `aria-label` は残るので、ツールチップとスクリーンリーダーからは何のボタンか辿れる。ネイティブ `<select>`（言語）は文字を `…` で省略できないため、最狭段では地球儀アイコンの上へ透明なまま重ねて当たり判定にする。
