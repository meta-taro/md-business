...
**開いているフォルダを見せ、そこを入口にする**。ツリーには第 1 階層しか出ないため、どこを開いているかは他に分かる場所がない。ヘッダー直下にフォルダ名を 1 行で置き、その行自体を履歴・開き直しの入口にする。

- **入口はラベルの付いた幅いっぱいの行**。小さなアイコンだけの入口は、押せること自体に気付かれず、その先の機能（履歴・前回の続きの記憶）ごと無かったことになる。
- **開き直す入口は 1 か所に集める**。ヘッダーにも「開く」を置くと、同じ操作が 2 か所に散り、どちらが何をするのか読み取る手間が増える。履歴に無いフォルダへは、履歴の一覧の続きから行けるようにする。
- **一覧は重ねる**。ツリーを押し下げると、選び直さずに閉じたときに元の見え方へ戻らない。

**開いていた場所を覚える**。同じフォルダへ戻るたびに第 1 階層まで畳まれ、開いていたファイルも閉じていると、毎回同じ手順で辿り直すことになる。展開状態と開いていたファイルを**フォルダごとに**覚え、次に開いたときへ引き継ぐ（エディタのワークスペース復元と同じ考え方）。

- **覚える単位はフォルダ**。行き来する数個のフォルダそれぞれで作業場所が違うため、直前の 1 つだけでは戻ったときに再現できない。覚えるフォルダ数は履歴と同じ範囲で止める。
- **同じフォルダを取り直したときは今の見え方を保つ**。保存・改名・ブランチ切替は「フォルダを開き直す」操作ではないので、記憶の側から組み直すと、その瞬間だけツリーが畳まれて場所を見失う。
- **全部畳んだ状態も記憶する**。畳んだのも操作の結果なので、空だからといって既定の展開へ戻さない。
- **消えたフォルダは記憶から落とす**。開けない行を残しても畳んだまま溜まり続けるだけ。履歴から消したフォルダの記憶も一緒に捨てる。
- **壊れた記憶で起動を止めない**。読めない値は記憶ごと捨てて、初めて開いたときと同じ状態から始める。
- **黙って戻さない**。復元は気付かれないと「勝手に開いた」に見え、覚えていること自体が伝わらない（自動保存を状態バーで見せているのと同じ理由）。記憶から実際に戻せたときだけ状態バーで一度知らせ、しばらくして自分で消える（閉じる操作を増やさない）。何も戻らなかったとき（対象が消えている・同じフォルダの取り直し）は知らせない。
- **履歴の行に「前回開いていたファイル」を添える**。知らせは流れて消えるので、覚えていることが常に見える場所も要る。開く前に何を触っていたかが分かると、同名フォルダの選び分けにも効く。
...
**右クリックメニュー**。行に対する操作は、ツールバーへ常設せず右クリックへ集める（ツリーは行の幅を情報に使いたいため）。

- **項目**: 名前の変更 / エクスプローラーで表示 / 名前をコピー / 相対パスをコピー / フルパスをコピー / リモートで開く（ファイルのみ・remote がある場合のみ）。
- **コピーは 3 種を並べる**。用途が違うので 1 つに寄せられない。名前は文中で文書を指すとき、相対パスは設計書の相互参照や frontmatter へ貼るとき（走査と同じ `/` 区切りのまま）、フルパスは他アプリへ渡すとき。
- **名前の変更はその場で行う**。別ダイアログを出すと、周りにどんな名前が並んでいるかが見えなくなる。対象の行だけを入力欄に差し替え、階層の位置はそのまま保つ。
- **拡張子の手前まで選択して開く**。直したいのはほとんどが名前の側で、`.md` / `.tsv` は残る。
- **移動には使わせない**。名前に区切り文字が入ると「改名」と「移動」が混ざり、取り消し方も変わる。区切り文字・`..`・OS が受け付けない文字はその場で理由を出して止める。
- **拡張子は `.md` / `.tsv` のまま**。変えるとツリーの走査対象から外れ、消えたように見えてしまう。
- **エラーは入力欄に出したまま閉じない**。衝突（同名あり）はその場で名前を変えれば済むので、閉じて理由を消すと押し直しになる。
- **確定は Enter、取り消しは Esc とフォーカス外し**。フォーカスが外れた時点で書き込むと、意図しない改名が起きる。
- **改名後は同じ行を追い掛ける**。開いていたファイル（そのファイル自身でも、親フォルダの改名でも）は新しいパスで開き直し、選択位置もその行へ戻す。
...
- **タブ帯 34px（`--tabbar-h`）**: Top bar の下・分割面の上。開いている文書が 1 枚も無ければ帯ごと出さない。§6.6。
...
**上限 12 枚**。超えたら最後に触ってから一番経ったタブを保存してから閉じる（`MAX_TABS` / `evictionTarget`）。

```
├──────────┬──────────────────────────────────────────────────────────────┤
│          │ ● 受発注API.md ✕ │ 001-login.tsv ✕ │ 請求書.md ✕ │          │ ← タブ帯 34px
├──────────┴──────────────────────────────────────────────────────────────┤
```

- **高さ**: 34px。地は `--bg-subtle`、下辺に 1px `--border`。エディター面と地色を変えて、帯であることを見せる。
- **手前のタブ**: 地を `--bg-app` にしてエディター面と繋げ、上辺に 2px `--accent` を敷く（`box-shadow: inset`）。文字は `--text-primary`。
- **奥のタブ**: 文字 `--text-tertiary`、地は透明。ホバーで `--bg-hover` + `--text-secondary`。
- **幅**: 1 枚あたり最大 200px。名前が入りきらなければ末尾を `…` で省く。全パスは `title` で読める。
- **未保存の印**: 名前の左に 6px の `--accent` の丸。閉じる前に保存する作りなので、印は「まだディスクに無い」ことだけを表す。
- **閉じる**: 右端に 20px の `✕`。**常には出さず**、そのタブにホバーしたときと手前のタブにだけ出す（12 枚並ぶと `✕` だけが目に付いて名前が読めない）。**中クリックでも閉じる**。ドラッグ並べ替えは持たない。
- **入りきらないとき**: 横スクロール（`overflow-x: auto`・細い軸）。畳んで隠すと、どれが開いているか分からなくなる。
- **検証グリッドの状態**: 選択セル・スクロール量は**文書ごと**に覚え、タブを戻ると同じ場所へ戻る。列幅・行高・折り返しは文書の宣言なので、タブが変わったらグリッドごと組み直す。
- **読み上げ**: 帯が `role="tablist"`、各タブが `role="tab"` + `aria-selected`。手前のタブだけが `tabindex="0"`。

### 6.7 図（本文に埋める折れ線 / 棒 / 円）

数字は表で持ち、図は本文に**書いた指定から起こす**。数字を図の中へ書き写さないので、表を直せば図もその場で変わる。
...
- **描き替えは本文の段階**。囲みを画像の記法（`![…](data:image/svg+xml;base64,…)`）へ置き換えてから組み立てる。画面・PDF・HTML 書き出し・サイト書き出しのどれも同じ本文を通るので、**画面には出るのに PDF には出ない**が起きない。
- **見た目**: 幅 640 × 高さ 320 を素の大きさとし、枠が狭ければ縮む。軸・目盛り・文字は文書の文字色（ライト `#1f2328` / ダーク `#e6edf3`）。系列の色は 6 色を順に使う（`#5b6ee1` `#2f9e78` `#d9822b` `#c14a4a` `#8a63c9` `#2c8fb5`）。目盛りは 1・2・5 の刻みで、必ず 0 を含む。横軸の見出しは 10 個を超えたら間引く。
- **描けないときは黙って空にしない**。理由を図の位置に 1 行の引用で出し、**書いた指定もそのまま残す**（消すと何を書いたのか本人にも分からなくなる）。表の中に数として読めないセルがあったときは、その分を空けて描いたうえで「何個あったか」を図の下に添える。
- **絵の中の色は呼ぶ側が決める**。画像として貼った絵は別の文書なので、文書側の文字色も書体も継がない。テーマを変えると図は描き直す。

### 6.8 書き出しに載せる本文は 1 か所で仕上げる

書き出し口は 3 つある（単一 HTML・画像・静的サイト）。どれも**組み上がった画面を通らない**ので、
画面の中を後から書き替える作りにしたものは、書き出しに載らない。

- **仕上げは 1 か所**（`preview/composeSource.ts`）。本文の画像を埋め、`chart` を描き、作図（Mermaid）を
  画像の記法に替えてから組む。口ごとに手順を写すと、片方だけ足し忘れて**画面には出るのに書き出すと消える**が起きる。
- **静的サイトでは画像を埋めない**。ページの組み立て側が画像をファイルとして運ぶため、埋めると同じ写真が
  ページの数だけ複製される。仕上げに渡す読み取り口を落とすことで、そこだけ手順から外す。
- **作図は画面と書き出しで経路が違う**。画面は打鍵のたびに組み直すので、描けたものから順に差し替わる
  今の作り（プレビューの中を描き替える）が速い。書き出しは 1 回きりなので本文の段階で画像にする。
  画像として貼るときは、大きさを `viewBox` の実寸で書き入れる（`width="100%"` は貼り先の幅が伝わらず決まらない）。
