...
| **v0.6.0** | 2026-06-下旬〜 | + **保存機能**（File System Access API、Ctrl+S 上書き / Ctrl+Shift+S 名前を付けて保存 / .bak 自動バックアップ）+ 請求書実務細部（振込手数料 / 締日 / 国税庁登録番号モーダル）。**PdM 2026-06-20 確定: ローカル md ⇔ Workspace Add-on アップロード入口の動線が成立した時点で v0.6.0 = Chrome 拡張次回 submit 候補**（即時審査は出さない）|
| **v0.7.0** | 2026-07〜 | + **`schema-test-spec`（検証シート）+ Workspace アドオン double-binding 完全版** — frontmatter `columns:` で列型 / enum values / visual を宣言 → Apps Script が DataValidation（プルダウン / 日付ピッカー / チェックボックス）+ ConditionalFormat（行全体 = `row_background` / セル単独 = `background`）+ setFrozenRows(1) を import 時に自動適用、export で frontmatter round-trip 復元。初版型 7 種（text / multiline_text / enum / date / number / checkbox / url）。**Markdown が真** の原則。シート側のイレギュラー（未知列 / enum 範囲外 / 型違反）は **検出 + 通知**（サイドバー警告 + `setNote` + 行背景色）。**PdM 2026-06-20 確定: v0.7.0 のリリース DoD は `md → Sheets セットアップ` + `validate` + `Sheets → md 書出し` の 3 ステップ往復が成立した時点。onEdit 自動同期 PUT は実装済だが PdM 動作確認 + 利用は v0.8 に後送り**（運用上「検証シートを各プロダクト担当に渡す」のが主動線で、自動 GitHub commit は副次要素）。完成と同時に **Phase 2 Google Workspace アドオン Marketplace submit**（Phase E 検証申請は Sensitive スコープ 3 個で 5〜7 週想定）。設計合意の詳細は `.claude/issues/002-v0.7.0-schema-test-spec.md` |
| v0.8.0 | 2026-08〜 | + `schema-api-spec`（API 設計書 — 詳細設計対応可）+ **DB 設計書フェーズ**（PdM 2026-06-18 確定: RDB と NoSQL を同時並行実装）: `schema-db-spec`（**RDB** — 型プルダウン + ER 図 + 制約 / index）+ `schema-nosql-db-spec`（**NoSQL** — JSON 連想配列構造を YAML 入れ子で宣言 → 枝記号付きツリー / インデント階層表示 + 型バッジ + `*required` マーカー、第一参照源 `coveooss/json-schema-for-humans`、engine: mongodb/firestore/dynamodb/cosmos/cassandra/neo4j/redis、PDF A4 縦、v1 は全展開固定、access pattern 表は副次要素） |
