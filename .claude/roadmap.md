...
| **v0.7.0** | 2026-07〜 | + **`schema-test-spec`（検証シート）** — frontmatter `columns:` で列型 / enum values / visual を宣言 → Apps Script が DataValidation（プルダウン / 日付ピッカー / チェックボックス）+ ConditionalFormat（行全体 = `row_background` / セル単独 = `background`）+ setFrozenRows(1) を import 時に自動適用、export で frontmatter round-trip 復元。初版型 7 種（text / multiline_text / enum / date / number / checkbox / url）。**Markdown が真** の原則（シート側で追加された未知列は export 時に捨てる）。完成と同時に **Phase 2 Google Workspace アドオン Marketplace submit**。設計合意フェーズの論点は `.claude/issues/002-v0.7.0-schema-test-spec.md` |
| v0.7.1 | v0.7.0 後続 | + Sheets onEdit → GitHub API → md commit の自動同期（v0.7.0 では手動 import/export ボタンのみ）|
