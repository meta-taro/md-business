...
| v0.5.0 | 2026-06-18〜19 想定 | + 請求書実務細部（振込手数料負担表記 / 締日基準の選択肢 / 端数処理ポリシー UI / 適格事業者登録番号メモ）+ Mermaid 代替手段検討（静的 SVG 埋め込み or vendoring） |
| v0.6.0 以降 | 2026-07〜（Phase 3 以降） | + `schema-test-spec`（テスト項目書）/ `schema-api-spec`（API 設計書 — 詳細設計対応可）/ `schema-db-spec`（DB 設計書）が PdM 次回優先候補（2026-06-17 セッション終了時の言及）。続いて `quotation` / `meeting-minutes` / `contract` / `resume` 等 |
