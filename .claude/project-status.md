...
- 次の本命: **schema-test-spec v0.7.0**（frontmatter スキーマ駆動でデータ検証 / 条件付き書式 / 日付ピッカー / ヘッダー固定を Sheets に流し込む double-binding 実装）
