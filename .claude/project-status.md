...
最終更新: 2026-06-18（**clasp v3 の `.claspignore` セマンティクス変更 fix**: PdM が `clasp push` を実行しても `Script is already up to date.` が返り Apps Script Editor にファイルが反映されない問題を解析。clasp v3 ソースコード（`@google/clasp@3.3.0`、`build/src/core/clasp.js`、`build/src/core/files.js`）を直接読んで原因特定: v3 では `.claspignore` のパターンが **`contentDir` (= `rootDir` で指定した `dist/`) 相対** で評価されるため、雛形時に書いた `**/**` + `!dist/Code.js` の dist プレフィックス付きホワイトリストが機能せず、結果的に **全ファイルが ignore → Tracked 空 → push 何もしない** 状態だった（v2 系では機能していたパターン）。`apps/google-workspace-addon/.claspignore` を v3 仕様に最小化（`node_modules/**` / `.git/**` / `.DS_Store` / `*.map` のみ ignore）+ submit guide / README に v3 セマンティクス解説とトラブルシュート行追加。PdM 側で `dist/.claspignore` を削除（無効な階層に作っていた一時ファイル）して `apps/google-workspace-addon/` 直下から `clasp push --force` 再実行で復旧見込み。ブラウザ別アカウント（個人 Gmail）混入問題も submit guide のトラブルシュート行に追加。前ブロック: 雛形双方向化 + 申請素材完成（commit `4a8b051`）
