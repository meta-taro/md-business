...
本アドオンは以下の 6 スコープを要求します。GitHub Push 機能のため、2 件は Google の **センシティブ（Sensitive）スコープ** に該当します。
...
manifest 残置スコープ（v0.7.1 から未使用 / 次バージョンで削除予定）:

- `script.scriptapp` — v0.7.0 以前で onEdit installable trigger の登録 / 削除に使用していましたが、Workspace Add-on context では Google の allowlist 制限により本スコープが動作しないことが確定し、v0.7.1 で手動 push ボタンに移行しました。現在は manifest にのみ残っており、ランタイム上は呼び出されません。
...
### `auth/script.scriptapp` (legacy, manifest-only — to be removed)

```
This scope was used in v0.7.0 and earlier to register and remove an onEdit installable trigger for the legacy auto-sync feature. In v0.7.1 we discovered that the Workspace Add-on context does not allow this scope (it is not on Google's allowlist for Add-ons), so we switched to a user-initiated "Push to GitHub" button and the auto-sync code path was deleted. The scope is currently still listed in appsscript.json but is never invoked at runtime; it will be removed from the manifest in the next minor release. If the reviewer prefers, we can remove it before publication.
```
...
- [ ] OAuth consent screen の **「アプリの確認」用フィールド** に上記「OAuth 検証申請 — スコープ用途説明」2 件（`spreadsheets` / `script.external_request`）を貼り付け。`script.scriptapp` セクションは legacy として併記、または申請前に appsscript.json から該当 scope を削除して 2 件に統一
...
すべてチェック後、Marketplace SDK → 「公開ステータス」→「**ドラフトを保存**」→「**確認のために送信**」。Sensitive scope 2 件（`spreadsheets` / `script.external_request`）のため Google レビューは **3〜5 週** を想定（`script.scriptapp` を申請前に manifest から外せば申請対象は 2 件で済む）。
