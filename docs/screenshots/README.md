...
| `desktop-zumen.png`               | 構成図。左に `zumen` の囲み、右に描かれた図が同時に見える位置まで送る   |
...
デスクトップアプリの 6 枚は**ダウンロードページ（`docs/download/index.html`）とトップページ
2 枚（`docs/index.html` / `docs/ja/index.html`）が参照している**。3 ページとも**ファイル名を直に
書いている**ので、**同じ名前で差し替えるなら置くだけでよいが、名前を増やす・消すときは 3 ページとも
直す**（1 ページだけ直すと、残りが壊れた画像のまま公開される）。ダウンロードページは 404 になった
figure を個別に伏せ、1 枚も無ければ「画面を見る」の節ごと出さないので、途中まで揃った状態でも
壊れない。トップページにはその仕掛けが無い。

- [x] `desktop-invoice.png` — 適格請求書を開いた画面（左にファイル一覧、右に請求書ビュー）
- [x] `desktop-editor.png` — Markdown の左右 2 画面ライブ編集
- [x] `desktop-zumen.png` — 構成図（左の `zumen` 囲みと右の図が両方見える状態）
