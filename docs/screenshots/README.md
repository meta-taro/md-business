...
（`docs/index.html`）の両方が参照している**。ダウンロードページはこのディレクトリへ置くだけで
**ページ側の編集は要らず、次の公開で自動的に出る**（1 枚も無い間は「画面を見る」の節ごと出ないので、
途中まで揃った状態でも壊れない）。トップページは節を隠す仕掛けを持たないので、**差し替えはよいが、
消すなら `docs/index.html` の `A look around the desktop app` も直す**。
