...
デスクトップアプリの 5 枚は**ダウンロードページ（`docs/download/index.html`）とトップページ
2 枚（`docs/index.html` / `docs/ja/index.html`）が参照している**。ダウンロードページはこのディレクトリへ
置くだけで**ページ側の編集は要らず、次の公開で自動的に出る**（1 枚も無い間は「画面を見る」の節ごと
出ないので、途中まで揃った状態でも壊れない）。トップページは節を隠す仕掛けを持たないので、
**差し替えはよいが、消すなら `docs/index.html` の `A look around the desktop app` と
`docs/ja/index.html` の「デスクトップアプリの中身」を両方直す**（片方だけ直すと、もう一方が
壊れた画像のまま公開される）。
