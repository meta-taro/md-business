...
| 承認済みドメイン | `github.com`, `example.com` |
| デベロッパー連絡先 | `you@example.com` |
...
GCP プロジェクトが Workspace 組織（例: `example.com`）配下にある場合、OAuth 同意画面の User type を **「内部」**（Internal）に切り替えると以下のメリットがある:
...
- 個人 Gmail（`*@gmail.com`）は Internal では使えない。社員アカウント（`*@example.com`）に限定される
...
3. Chrome に個人アカウントと社員アカウントが同時ログインしているとタブごとに context が flip して事故が起きやすい → **example.com 業務専用の Chrome プロファイル分離を推奨**
...
| Chrome のタブ毎にアカウントが flip する | 複数アカウント同時ログイン | example.com 業務専用 Chrome プロファイル分離 |
...
| Apps Script Editor を開いた時にブラウザが別の Google アカウント（個人 Gmail 等）でログイン中 | clasp は CLI 側のアカウント（業務）でプロジェクトを作るが、ブラウザのデフォルトプロファイルが個人アカウント | URL 末尾に `?authuser=you@example.com` を付ける / Chrome 専用プロファイル分離。**「アクセス権限をリクエスト」ボタンは絶対押さない**（個人アカウントから業務スクリプトへの権限申請メールが走る） |
...
| Chrome のタブ毎にアカウントが flip / OAuth が想定外のアカウントで進む | 個人 Gmail + 業務アカウントが Chrome に同時ログイン | 不要アカウントをログアウト / Chrome の「ユーザー追加」で example.com 業務専用プロファイルを作成して恒久分離 |
