...
| export | `#@ export <名前> [columns=…] [blank=…] [newline=…]` | 提出様式。先方の様式に合わせた列順・空欄の埋め方・改行の畳み方を宣言する。**正本は書き換えない**（書き出しのときだけ効く）。 |
...
```
#@ export 提出用 columns=No.,項目,結果 blank=- newline=space
#@ export 社内共有
```
...
- 名前が無い・知らないオプション（`columns` / `blank` / `newline` 以外）・列定義に無い列名・
  知らない改行の扱い、のいずれかならその宣言ごと捨てられる。半端に効いた様式で提出物が
  出るより、様式が出てこないほうが気づける（`#@ link` / `#@ computed` と同じ判断）。
