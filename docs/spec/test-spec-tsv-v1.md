...
| export | `#@ export <名前> [columns=…] [blank=…] [newline=…] [key=…]` | 提出様式。先方の様式に合わせた列順・空欄の埋め方・改行の畳み方を宣言する。**正本は書き換えない**（書き出しのときだけ効く）。`key=` があれば、返ってきた提出物を正本へ戻せる。 |
...
- 名前が無い・知らないオプション（`columns` / `blank` / `newline` / `key` 以外）・列定義に
  無い列名・知らない改行の扱い、のいずれかならその宣言ごと捨てられる。半端に効いた様式で
...
注釈は持ち出し（`#@ export`）にも出す。控え（`#@ hidden`）と `備考` 列が「相手に見せない
メモ」を既に受け持っており、注釈まで内輪扱いにすると、**見えていないものが黙って提出物から
落ちる**。
