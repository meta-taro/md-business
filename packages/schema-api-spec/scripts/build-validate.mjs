...
const schemaPath = path.join(root, 'src/api-spec.schema.json');
...
if (/require\(/.test(patched) || /__cjs_\d+/.test(patched)) {
  throw new Error(
    '[schema-api-spec] unexpected require() or __cjs_N binding left in compiled validator',
  );
}
...
console.log(`[schema-api-spec] wrote ${path.relative(root, outJs)}`);
