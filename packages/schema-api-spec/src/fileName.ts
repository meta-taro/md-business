 ...
 * curly braces and refer to api-spec fields:
 *
 ...
 *
 * Falls back to the default rule `API設計書_{文書番号}_v{版}` when no
 ...
 * safe to save.
 */
export function renderApiSpecFileName(apiSpec: ApiSpec, template?: string): string {
  const tpl = template?.trim() || 'API設計書_{文書番号}_v{版}';
  const now = todayLocal();
  const tokens: Record<string, string> = {
    '文書番号': apiSpec.documentNumber ?? '',
    'documentNumber': apiSpec.documentNumber ?? '',
    'タイトル': apiSpec.title ?? '',
    'title': apiSpec.title ?? '',
    '版': apiSpec.version ?? '',
    'version': apiSpec.version ?? '',
    'ステータス': apiSpec.status ?? '',
    'status': apiSpec.status ?? '',
    'プロトコル': apiSpec.protocol ?? '',
    'protocol': apiSpec.protocol ?? '',
    '発行日': apiSpec.issueDate ?? '',
    'issueDate': apiSpec.issueDate ?? '',
    '発行日YMD': stripDashes(apiSpec.issueDate ?? ''),
    'issueYMD': stripDashes(apiSpec.issueDate ?? ''),
    'YMD': now.ymd,
    'date': now.iso,
    '今日': now.iso,
  };
