...
function buildMinimalNosqlDbSpec(): Record<string, unknown> {
  return {
    ...
    collections: [
      {
        path: 'users',
        docIdStrategy: 'auto',
        shape: { uid: { type: 'string', required: true } },
      },
    ],
  };
}
...
describe('validate.compiled.js runtime behaviour after standalone inlining', () => {
  it('accepts a valid nosql-db-spec frontmatter object', async () => {
    const mod = await import('../dist/validate.compiled.js');
    const validate: any = (mod as any).default ?? (mod as any).validate;
    expect(validate(buildMinimalNosqlDbSpec())).toBe(true);
  });

  it('rejects an invalid date (format: date)', async () => {
    const mod = await import('../dist/validate.compiled.js');
    const validate: any = (mod as any).default ?? (mod as any).validate;
    expect(validate({ ...buildMinimalNosqlDbSpec(), issueDate: '2026-13-40' })).toBe(false);
  });

  it('rejects an empty string (minLength: 1 via ucs2length)', async () => {
    const mod = await import('../dist/validate.compiled.js');
    const validate: any = (mod as any).default ?? (mod as any).validate;
    expect(validate({ ...buildMinimalNosqlDbSpec(), documentNumber: '' })).toBe(false);
  });
});
