...
const schemaPath = path.join(root, 'src/nosql-db-spec.schema.json');
...
if (/require\(/.test(patched) || /__cjs_\d+/.test(patched)) {
  throw new Error(
    '[schema-nosql-db-spec] unexpected require() or __cjs_N binding left in compiled validator',
  );
}
...
console.log(`[schema-nosql-db-spec] wrote ${path.relative(root, outJs)}`);
