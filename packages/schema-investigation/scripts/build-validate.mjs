...
const schemaPath = path.join(root, 'src/investigation.schema.json');
...
const patched = rawCode
  .replace(/require\(\s*"ajv\/dist\/runtime\/ucs2length"\s*\)\.default/g, '__md_ucs2length')
  .replace(
    /require\(\s*"ajv\/dist\/runtime\/ucs2length"\s*\)/g,
    '({ default: __md_ucs2length })',
  )
  .replace(/require\(\s*"ajv-formats\/dist\/formats"\s*\)/g, '__md_ajv_formats');

if (/require\(/.test(patched) || /__cjs_\d+/.test(patched)) {
  throw new Error(
    '[schema-investigation] unexpected require() or __cjs_N binding left in compiled validator',
  );
}
...
console.log(`[schema-investigation] wrote ${path.relative(root, outJs)}`);
