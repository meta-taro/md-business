...
// `normalizeTestSpecFrontmatter` is exported on its own, so it is reachable
...
describe('normalizeTestSpecFrontmatter — nesting depth', () => {
  ...
  it('stops descending past the depth limit instead of overflowing', () => {
    let result: ReturnType<typeof normalizeTestSpecFrontmatter> | undefined;
    expect(() => {
      result = normalizeTestSpecFrontmatter({ 著者: nestArrays(50_000) });
    }).not.toThrow();
