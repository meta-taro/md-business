// Precompile the spec JSON Schema into a standalone ES module so the browser
// bundle never needs runtime `new Function()` (CSP-unsafe under MV3).
//
...
const schemaPath = path.join(root, 'src/spec.schema.json');
...
// Same require()->import hoisting as schema-invoice. Ajv standalone with
// esm:true still emits CommonJS require() calls for ucs2length and
// ajv-formats; rewrite them so Vite/Rollup can inline under MV3 CSP.
...
console.log(`[schema-spec] wrote ${path.relative(root, outJs)}`);
