...
function buildMinimalDbSpec(): Record<string, unknown> {
  return {
    schema: 'db-spec/v1',
...
describe('validate.compiled.js runtime behaviour after standalone inlining', () => {
  it('accepts a valid db-spec frontmatter object', async () => {
    const mod = await import('../dist/validate.compiled.js');
    const validate: any = (mod as any).default ?? (mod as any).validate;
    expect(validate(buildMinimalDbSpec())).toBe(true);
  });

  it('rejects an invalid date (format: date)', async () => {
    const mod = await import('../dist/validate.compiled.js');
    const validate: any = (mod as any).default ?? (mod as any).validate;
    expect(validate({ ...buildMinimalDbSpec(), issueDate: '2026-13-40' })).toBe(false);
  });

  it('rejects an empty string (minLength: 1 via ucs2length)', async () => {
    const mod = await import('../dist/validate.compiled.js');
    const validate: any = (mod as any).default ?? (mod as any).validate;
    expect(validate({ ...buildMinimalDbSpec(), documentNumber: '' })).toBe(false);
  });
});
