...
const schemaPath = path.join(root, 'src/db-spec.schema.json');
...
if (/require\(/.test(patched) || /__cjs_\d+/.test(patched)) {
  throw new Error(
    '[schema-db-spec] unexpected require() or __cjs_N binding left in compiled validator',
  );
}
...
console.log(`[schema-db-spec] wrote ${path.relative(root, outJs)}`);
