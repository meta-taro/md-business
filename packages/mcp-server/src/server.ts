...
- フォルダが **web モード**を名乗っているとき（\`md-business.yml\`）は、サイトの部品
  （HTML / CSS / JS など）もこのワークスペースの中身になる。部品には形が無く素のファイル編集でも
  書けてしまうが、それでは画面にも操作ログにも出ない。**list_site_files** で今あるものを見て、
  直すものは **read_site_file** で置いてあるまま読んでから **write_site_file** で書き戻す
