...
| `list_site_files` | web モードを名乗っているフォルダにあるサイトの部品の一覧を返す。何が既にあるかを掴んでから読み書きする |
...
| `capture_window` | デスクトップアプリの窓を撮って画像で返す。写るのはこのアプリの窓だけ |
...
`open_document` / `list_open_documents` / `close_document` / `export_pdf` / `capture_window` はデスクトップアプリから
...
`capture_window` はアプリが実際に何を描いているかを見るための口で、文書の中身を読むためのものではない
（読むなら `read_document` のほうが速く、確かめられる）。撮るのは画面ではなくアプリが抱えている
webview なので、**手前に重なっている他のアプリは写らない**し、隠れていても撮れる。
既定では長辺 1400 まで縮める（`maxEdge` で 200〜4000 の間を指定できる）。小さい窓は引き伸ばさない。
返る画像は**利用者が今見ている画面そのもの**なので、Issue や PR のような外から見えるところへ
貼る前に本人へ確認する。
...
`declare_web_mode` と `list_site_files` / `read_site_file` / `write_site_file` は、業務文書ではなく
**サイトの部品**（HTML / CSS / JS など）を扱うための 4 本。既定のフォルダは業務文書として扱われ、サイトの部品は一覧に出ない。`declare_web_mode` で
...
中身は渡されたまま置く（組み立て直さない）ので、出来上がりは利用者が開いている面にそのまま映る。
何が既にあるかは `list_site_files` で分かる（業務文書と検証シートは `search_documents` 側に出る）。
既にあるファイルを直すときは `read_site_file` で読んでから書き戻す。こちらも置いてあるままを返す——
伏せ字や行の切り詰めが混ざると、書き戻した時点で触っていないはずの箇所まで書き換わってしまう。

Astro / Vite のように組み上げてから出す作りでは、素のファイルを映しても出来上がりにならない。
その待ち受けを自分で動かしているなら、在り処を `md-business.yml` の `web.devServer` に書く
（`web: { devServer: http://localhost:4321 }`）。書けば利用者の面がそこを映す。
**アプリはその処理を起動しない**——動かすのも止めるのも書いた側の仕事で、
止めたままにすると面は「まだ応えていない」と出る。受け付けるのはこの PC の中の在り処だけで、
外向きの住所を書いても映さない（リポジトリの中のファイルが、アプリに何を映させるかを決められては困る）。
...
### 1 台の PC で複数のワークスペースを扱う

**1 つのサーバーが見るのは 1 つのフォルダ**。第 2 引数以降のフォルダは受け付けず、
理由を出して終了コード 1 で終わる（黙って捨てると、渡した側は「両方見た」と思ったまま結果を読む）。

複数のプロダクトを扱うときは、**フォルダごとにサーバーを登録し、登録名を行き先と揃える**。
...
```json
{
  "mcpServers": {
    "md-business-shop": {
      "command": "node",
      "args": ["<repo>/packages/mcp-server/dist/bin.js", "<workspace-shop>"]
    },
    "md-business-billing": {
      "command": "node",
      "args": ["<repo>/packages/mcp-server/dist/bin.js", "<workspace-billing>"]
    }
  }
}
```

登録はできれば**プロジェクト単位**（そのリポジトリの `.mcp.json`）に置く。
全体設定に 2 本並べると、片方のプロジェクトで作業しているときにもう片方が常に見えていて、
行き先を選ぶ判断が毎回発生する。

登録名が中身とずれていても**書き込みは成功する**ので、取り違えは誰にも見えない。
そのため、接続時の案内文の先頭と `search_documents` の応答が接続先の実パスを名乗る。
`--health <workspace>` で、その名前がどのフォルダを指しているかを先に確かめられる。
...
### 同じフォルダを複数のプロセスが開いたとき

1 台の PC で複数のエージェントを動かすと、同じフォルダを別々のサーバープロセスが指す。
検証シートの行の書き込みは「全文を読む → 1 行足す / 差し替える → 全文を書き戻す」なので、
読んでから書くまでの間に他所が書くと、そのまま踏み潰しうる。二段で防いでいる。

1. **順番を取り合う** — 読み込みから書き戻しまでの間、対象ファイルの隣に錠
   （`.<ファイル名>.lock`）を置く。既にあれば少し待って試し直し、5 秒取れなければ
   理由を返して諦める。錠を握ったままプロセスが消えた場合に備え、10 秒放置された錠は奪う。
2. **書く直前に突き合わせる** — 錠を取らない相手（エディタ、デスクトップアプリ、
   ほかのツール）が書き換えていないか、書き込みの直前に読み直して確かめる。
   変わっていれば上書きせず、読み直しを促すエラーを返す。

1 は md-business のサーバー同士でのみ効き、2 はそれ以外にも効く。
2 の確認から書き込みまでのわずかな隙間は残るので、**同じシートを人とエージェントが
同時に編集するのは避ける**のが確実。
